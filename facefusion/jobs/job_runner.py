...
def run_step(job_id : str, step_index : int, step : JobStep, process_step : ProcessStep) -> bool:
        ...
        if job_manager.set_step_status(job_id, step_index, 'started') and process_step(job_id, step_index, step_args):
                ...
                if not state_manager.get_item('skip_merge'):
...
def run_steps(job_id : str, process_step : ProcessStep) -> bool:
        ...
        if steps:
                ...
                for index, step in enumerate(steps):
                        is_first = index == 0
                        is_last = index == step_total - 1
                        state_manager.set_item('skip_extract', not is_first)
                        state_manager.set_item('skip_merge', not is_last)
                        if not is_last:
                                state_manager.set_item('keep_temp', True)
                        else:
                                state_manager.set_item('keep_temp', step.get('args').get('keep_temp'))
...
def finalize_steps(job_id : str) -> bool:
        ...
        for output_path, temp_output_paths in output_set.items():
                valid_paths = [ path for path in temp_output_paths if is_file(path) ]
                if not valid_paths:
                        continue
                if are_videos(valid_paths):
                        if not concat_video(output_path, valid_paths):
                                return False
                if are_images(valid_paths):
                        for temp_output_path in valid_paths:
